...
<td>
  {if $child[i].rows}
    {datagrid2 parent=$child[i]._fkey rows=$child[i].rows key=$child[i].key title=$child[i].title dd=$child[i].dd maxcols=$child[i].maxcols|default:5 maxrows=$child[i].maxrows|default:8 paginate=false cmd=true name=$child[i].name object=$child[i].name options=$child[i]._options is_child=true num_rows=$child[i].num_rows}
