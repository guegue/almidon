...
define('DRHEADERCELL', '<th><a class="dgheader_link" href="_SELF_?o=_FORM_&id=_IDOPT_&_FORM_sort=_FIELD_">_LABEL_</a></th>');
...
function getsubtotal() {
  global $_old_row, $group, $_key, $_val, $dd, $rows, $subtotals, $counter;
  $_tmp = "<td><font color=red>". $_old_row[$group]  . " (" . $counter[$_old_row[$group]] . ") </font></td>";
  if ($subtotals) {
...
function smarty_function_datareport2($params, &$smarty)
{
global $_old_row, $group, $_key, $_val, $dd, $rows, $subtotals, $counter;
    ...
    foreach ($row as $_key=>$_val) {
      ...
      switch ($dd[$_key]['type']) {
        ...
        case 'image':
          ...
          break;
        case 'link':
          $_tmp = smarty_modifier_truncate($_val, 500);
          break;
        default:
          $_tmp = smarty_modifier_truncate($_val, 150);
      }
  ...
  $_html_result = preg_replace("/_IDOPT_/", $idopt, $_html_result);
