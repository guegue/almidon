...
function smarty_modifier_url($string)
{
    ...
    $string = preg_replace("/(^|\s|\(|>)www\.(.*?)(\s|\)|\,|<|$)/", "$1<a target=\"_blank\" href=\"http://www.$2\">www.$2</a>$3", $string);
    $string = preg_replace("/(^|\s|\(|>)(\w+?)@(\w+?)\.(\w+?)(\s|\)|\,|<|$)/", "$1<a target=\"_blank\" href=\"mailto:$2@$3.$4\">$2@$3.$4</a>$5", $string);
    $string = preg_replace("/(^|\s|\()((\w|\.)+?)@((\w|-)+?)\.(\w+?)(\s|\)|\,|<|$)/", "$1<a target=\"_blank\" href=\"mailto:$2@$4.$6\">$2@$4.$6</a>$7", $string);
    $string = preg_replace("/(^|\s|\()(\w+?)@(\w+?)\.(\w+?)\.(\w+?)(\s|\)|\,|<|$)/", "$1<a target=\"_blank\" href=\"mailto:$2@$3.$4.$5\">$2@$3.$4.$5</a>$6", $string);
    $string = preg_replace("/(^|\s|\()(\w+?)@(\w+?)\.(\w+?)\.(\w+?)\.(\w+?)(\s|\)|\,|<|$)/", "$1<a target=\"_blank\" href=\"mailto:$2@$3.$4.$5.$6\">$2@$3.$4.$5.$6</a>$7", $string);
    #error_log($string);
    return $string;
}
