    ...
    if ($_REQUEST['f'] == $name && $_REQUEST['action'] == 'mod' && $_chosen) {
      ...
      foreach ($row as $_key=>$_val) {
        ...
        switch ($dd[$_key]['type']) {
          ...
          case 'img':
            $_tmp = '';
            if ($_val) $_tmp = '<input type="checkbox" checked name="' . $_key . '_keep" /> Conservar archivo actual (' . $_val . ')<br /><img src="/cms/thumb/pic/50/' . DOMAIN . '/' . $table . '/' . $_val . '" alt="' . $_val  . '" width="50" border="0" /><br />';
      ...
      foreach ($row as $_key=>$_val) {
        ...
        switch ($dd[$_key]['type']) {
          ...
          case 'image':
            if ($_val) {
              if (THUMBNAILING)
                $_tmp = '<a href="javascript:openimage(\'http://' . DOMAIN . '/files/' . $table . '/' . $_val . '\',\'Imagen: ' . $_val . '\')"><img src="/cms/thumb/pic/50/' . DOMAIN . '/' . $table . '/' . $_val . '" alt="' . $_val  . '" width="50" border="0" /></a>';
              else
