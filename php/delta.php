...
class Data {
  ...
  function Data () {
    global $DSN;
    if ($DSN)
      $this->database = DB::connect ($DSN, true);
    else
      $this->database = DB::connect (DSN, true);
...
class Table extends Data {
  ...
  function parsevar($tmpvar, $type = 'string', $html = false) { 
    ...
    if ($type == 'string' && !$html) {
      $tmpvar = strip_tags($tmpvar, "<br><p><h1><h2><h3><b><i><div><span><img1><img2><img3><strong><li><ul><table><tbody><tr><td><a>");
  ...
  function readEnv() {
    ...
    foreach($this->definition as $column) {
      if ($column['type'] != 'external' || $column['type'] != 'auto') {
        ...
        } elseif ($column['type'] == 'date' || $column['type'] == 'datetime' || $column['type'] == 'time') {
          $date = ''; $time = '';
      ...
      switch($column['type']) {
        ...
        case 'image':
          if ($this->files[$column['name']]) {
            ...
            $sizes = explode(',',$column['extra']);
            foreach($sizes as $size) {
              $picurl = URL . "/cms/pic/" . $size . "/" . $this->name . "/" . rawurlencode($filename);
              $thumbf = ROOTDIR . "/pix/" . $year . "/" . $month . "/" . $size . "_" . $filename;
